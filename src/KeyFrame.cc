...
KeyFrame::KeyFrame() :
    ...
    mThDepth(0),
    N(0),
    mvKeys(static_cast<vector<cv::KeyPoint> >(NULL)), /*mDescriptors(NULL),*/
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) :
  ...
  this->Tlr_ = cv::Matx44f(mTlr.at<float>(0, 0),
                           mTlr.at<float>(0, 1),
                           mTlr.at<float>(0, 2),
                           mTlr.at<float>(0, 3),
                           mTlr.at<float>(1, 0),
...
void KeyFrame::SetPose(const cv::Mat &Tcw_) {
  ...
  //Static matrices
  this->Tcw_ = cv::Matx44f(Tcw.at<float>(0, 0),
                           Tcw.at<float>(0, 1),
                           Tcw.at<float>(0, 2),
                           Tcw.at<float>(0, 3),
                           Tcw.at<float>(1, 0),
  ...
  this->Twc_ = cv::Matx44f(Twc.at<float>(0, 0),
                           Twc.at<float>(0, 1),
                           Twc.at<float>(0, 2),
                           Twc.at<float>(0, 3),
                           Twc.at<float>(1, 0),
...
void KeyFrame::UpdateConnections(bool upParent) {
  ...
  for (map<KeyFrame *, int>::iterator mit = KFcounter.begin(),
    ...
    if (!upParent)
      cout << "  UPDATE_CONN: KF " << mit->first->mnId << " ; num matches: "
           << mit->second << endl;
    if (mit->second > nmax) {
      nmax = mit->second;
      pKFmax = mit->first;
