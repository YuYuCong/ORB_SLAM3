...
bool Tracking::PredictStateIMU() {
  ...
  if (mbMapUpdated && mpLastKeyFrame) {
    ...
    cv::Mat Rwb2 = IMU::NormalizeRotation(Rwb1
                                              * mpImuPreintegratedFromLastKF->GetDeltaRotation(
                                                  mpLastKeyFrame->GetImuBias()));
    cv::Mat twb2 = twb1 + Vwb1 * t12 + 0.5f * t12 * t12 * Gz + Rwb1
        * mpImuPreintegratedFromLastKF->GetDeltaPosition(mpLastKeyFrame->GetImuBias());
    cv::Mat Vwb2 = Vwb1 + t12 * Gz + Rwb1
        * mpImuPreintegratedFromLastKF->GetDeltaVelocity(mpLastKeyFrame->GetImuBias());
    ...
    cv::Mat Rwb2 = IMU::NormalizeRotation(Rwb1
                                              * mCurrentFrame.mpImuPreintegratedFrame->GetDeltaRotation(
                                                  mLastFrame.mImuBias));
    cv::Mat twb2 = twb1 + Vwb1 * t12 + 0.5f * t12 * t12 * Gz + Rwb1
        * mCurrentFrame.mpImuPreintegratedFrame->GetDeltaPosition(mLastFrame.mImuBias);
    cv::Mat Vwb2 = Vwb1 + t12 * Gz + Rwb1
...
void Tracking::ComputeGyroBias(const vector<Frame *> &vpFs,
  ...
  for (int i = 1; i < N; i++) {
    ...
    assert(fabs(
        pF2->mTimeStamp - pF1->mTimeStamp - pF2->mpImuPreintegratedFrame->dT)
               < 0.01);
...
void Tracking::Track() {
  ...
  if (mState != NO_IMAGES_YET) {
    if (mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp) {
      ...
      cout << "id last: " << mLastFrame.mnId << "    id curr: "
           << mCurrentFrame.mnId << endl;
      if (mpAtlas->isInertial()) {

        if (mpAtlas->isImuInitialized()) {
          cout
              << "Timestamp jump detected. State set to LOST. Reseting IMU integration..."
              << endl;
  ...
  if (mState == NOT_INITIALIZED) {
    if (mSensor == System::STEREO || mSensor == System::RGBD
      ...
      MonocularInitialization(); // monocular init
    }
    ...
    if (bOK || mState == RECENTLY_LOST) {
      ...
      if (!mLastFrame.mTcw.empty() && !mCurrentFrame.mTcw.empty()) {
        ...
        mLastFrame.GetRotationInverse().copyTo(LastTwc.rowRange(0,
                                                                3).colRange(0,
                                                                            3));
...
void Tracking::StereoInitialization() {
  if (mCurrentFrame.N > 500) {
    ...
    if (mSensor == System::IMU_STEREO) {
      ...
      mCurrentFrame.SetImuPoseVelocity(Rwb0,
                                       twb0,
    ...
    if (!mpCamera2) {
      for (int i = 0; i < mCurrentFrame.N; i++) {
        ...
        if (z > 0) {
          ...
          MapPoint
              *pNewMP = new MapPoint(x3D, pKFini, mpAtlas->GetCurrentMap());
...
void Tracking::CreateInitialMapMonocular() {
  ...
  mVelocity = cv::Mat();
  Eigen::Vector3d
      phi = LogSO3(Converter::toMatrix3d(deltaT.rowRange(0, 3).colRange(0, 3)));
...
void Tracking::UpdateLastFrame() {
  ...
  for (size_t j = 0; j < vDepthIdx.size(); j++) {
    ...
    if (bCreateNew) {
      ...
      MapPoint
          *pNewMP = new MapPoint(x3D, mpAtlas->GetCurrentMap(), &mLastFrame, i);
      ...
      if (pMP) {
        if (!pMP->isBad()) {
          ...
          for (map<KeyFrame *, tuple<int, int>>::const_iterator
                   it = observations.begin(), itend = observations.end();
               it != itend; it++)
  ...
  for (int i = 0; i < nKFs; i++) {
    ...
    if (pKF->isBad())
      ...
      if (nmatches < 15) {
        ...
        MLPnPsolver
            *pSolver = new MLPnPsolver(mCurrentFrame, vvpMapPointMatches[i]);
...
void Tracking::UpdateFrameIMU(const float s,
  ...
  for (list<cv::Mat>::iterator lit = mlRelativeFramePoses.begin(),
           lend = mlRelativeFramePoses.end(); lit != lend;
       lit++, lRit++, lbL++) {
    if (*lbL)
      continue;
  ...
  if (mCurrentFrame.mpImuPreintegrated) {
    ...
    mCurrentFrame.SetImuPoseVelocity(
        ...
        twb1 + Vwb1 * t12 + 0.5f * t12 * t12 * Gz + Rwb1
...
void Tracking::CreateNewMapPoints() {
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    matcher.SearchForTriangulation(mpLastKeyFrame,
                                   pKF2,
                                   F12,
                                   vMatchedIndices,
                                   false);
    ...
    for (int ikp = 0; ikp < nmatches; ikp++) {
      ...
      cv::Mat xn1 =
          (cv::Mat_<float>(3, 1) << (kp1.pt.x - cx1) * invfx1, (kp1.pt.y - cy1)
              * invfy1, 1.0);
      cv::Mat xn2 =
          (cv::Mat_<float>(3, 1) << (kp2.pt.x - cx2) * invfx2, (kp2.pt.y - cy2)
              * invfy2, 1.0);
      ...
      if (bStereo1)
        cosParallaxStereo1 = cos(2 * atan2(mpLastKeyFrame->mb / 2,
      ...
      MapPoint
          *pMP = new MapPoint(x3D, mpLastKeyFrame, mpAtlas->GetCurrentMap());
