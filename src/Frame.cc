...
void Frame::UpdatePoseMatrices() {
  ...
  mRcwx = cv::Matx33f(mRcw.at<float>(0, 0),
                      mRcw.at<float>(0, 1),
                      mRcw.at<float>(0, 2),
                      mRcw.at<float>(1, 0),
                      mRcw.at<float>(1, 1),
...
vector<size_t> Frame::GetFeaturesInArea(const float &x,
  ...
  for (int ix = nMinCellX; ix <= nMaxCellX; ix++) {
    for (int iy = nMinCellY; iy <= nMaxCellY; iy++) {
      const vector<size_t>
          vCell = (!bRight) ? mGrid[ix][iy] : mGridRight[ix][iy];
      if (vCell.empty())
        continue;
...
void Frame::ComputeStereoMatches() {
  ...
  for (int iL = 0; iL < N; iL++) {
    ...
    if (bestDist < thOrbDist) {
      ...
      if (iniu < 0 ||
          endu >= (*mpORBextractorLeft->GetImagePyramid())[kpL.octave].cols)
        continue;

      for (int incR = -L; incR <= +L; incR++) {
        cv::Mat IR =
            (*mpORBextractorLeft->GetImagePyramid())[kpL.octave]
                 .rowRange(scaledvL - w, scaledvL + w + 1)
...
Frame::Frame(const cv::Mat &imLeft,
  ...
  mTrlx = cv::Matx34f(Rrl.at<float>(0, 0),
                      Rrl.at<float>(0, 1),
                      Rrl.at<float>(0, 2),
                      trl.at<float>(0),
                      Rrl.at<float>(1, 0),
                      ...
                      Rrl.at<float>(2, 2),
                      trl.at<float>(2));
  mTlrx = cv::Matx34f(mRlr.at<float>(0, 0),
                      mRlr.at<float>(0, 1),
                      mRlr.at<float>(0, 2),
                      mtlr.at<float>(0),
                      mRlr.at<float>(1, 0),
...
void Frame::ComputeStereoFishEyeMatches() {
  ...
  vector<cv::KeyPoint>
      stereoRight(mvKeysRight.begin() + monoRight, mvKeysRight.end());
