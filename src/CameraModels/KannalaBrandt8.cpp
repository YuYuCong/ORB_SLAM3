...
Eigen::Matrix<double,
              2,
              3> KannalaBrandt8::projectJac(const Eigen::Vector3d &v3D) {
...
bool KannalaBrandt8::ReconstructWithTwoViews(const std::vector<cv::KeyPoint> &vKeys1,
  ...
  cv::Mat D = (cv::Mat_<float>(4, 1)
      << mvParameters[4], mvParameters[5], mvParameters[6], mvParameters[7]);
  ...
  return tvr->Reconstruct(vKeysUn1,
                          vKeysUn2,
                          vMatches12,
                          R21,
                          t21,
...
cv::Matx33f KannalaBrandt8::toK_() {
  cv::Matx33f K{mvParameters[0], 0.f, mvParameters[2], 0.f, mvParameters[1],
...
bool KannalaBrandt8::epipolarConstrain(GeometricCamera *pCamera2,
                                       ...
                                       const float unc) {
  cv::Mat p3D;
  return this->TriangulateMatches(pCamera2,
                                  kp1,
                                  kp2,
                                  R12,
                                  t12,
...
bool KannalaBrandt8::epipolarConstrain_(GeometricCamera *pCamera2,
                                        const cv::KeyPoint &kp1,
                                        const cv::KeyPoint &kp2,
                                        ...
                                        const float unc) {
  cv::Matx31f p3D;
  return this->TriangulateMatches_(pCamera2,
                                   kp1,
                                   kp2,
                                   R12,
                                   t12,
...
void KannalaBrandt8::Triangulate_(const cv::Point2f &p1,
                                  const cv::Point2f &p2,
