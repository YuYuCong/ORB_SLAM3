...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs,
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    for (map < KeyFrame * , tuple < int, int >> ::const_iterator
        mit = observations.begin(); mit != observations.end();
    mit++) {
      ...
      if (pKF->mpCamera2) {
        ...
        if (rightIndex != -1 && rightIndex < pKF->mvKeysRight.size()) {
          ...
          ORB_SLAM3::EdgeSE3ProjectXYZToBody
...
void Optimizer::FullInertialBA(Map *pMap,
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (pKFi->bImu && pKFi->mPrevKF->bImu) {
        ...
        g2o::HyperGraph::Vertex
            *VV1 = optimizer.vertex(maxKFid + 3 * (pKFi->mPrevKF->mnId) + 1);
        ...
        g2o::HyperGraph::Vertex
            *VV2 = optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 1);
        ...
        if (!bInit) {
          ...
          egr->setVertex(1, VG2);
          cv::Mat cvInfoG =
              pKFi->mpImuPreintegrated->C.rowRange(9, 12).colRange(9,
                                                                   12).inv(cv::DECOMP_SVD);
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (nLoopId == 0) {
      ...
      Converter::toCvMat(VP->estimate().Rcw[0]).copyTo(pKFi->mTcwGBA.rowRange(0,
                                                                              3).colRange(
          0,
          3));
      Converter::toCvMat(VP->estimate().tcw[0]).copyTo(pKFi->mTcwGBA.rowRange(0,
                                                                              3).col(
          3));
    ...
    if (pKFi->bImu) {
      VertexVelocity *VV = static_cast<VertexVelocity *>(optimizer.vertex(
          maxKFid + 3 * (pKFi->mnId) + 1));
      if (nLoopId == 0) {
      ...
      VertexAccBias *VA;
      if (!bInit) {
        VG = static_cast<VertexGyroBias *>(optimizer.vertex(
            maxKFid + 3 * (pKFi->mnId) + 2));
        VA = static_cast<VertexAccBias *>(optimizer.vertex(
            maxKFid + 3 * (pKFi->mnId) + 3));
      } else {
      ...
      if (pMP) {
        ...
        if (!pFrame->mpCamera2) {
          ...
          if (pFrame->mvuRight[i] < 0) {
            ...
            ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose
                *e = new ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose();
          ...
          } else  // Stereo observation
          {
            ...
            g2o::EdgeStereoSE3ProjectXYZOnlyPose
                *e = new g2o::EdgeStereoSE3ProjectXYZOnlyPose();
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF,
  ...
  for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
    ...
    map < KeyFrame * , tuple < int, int >> observations =
                                        (*lit)->GetObservations();
    for (map < KeyFrame * , tuple < int, int >> ::iterator mit =
                                                               observations.begin(),
        mend = observations.end(); mit != mend;
    mit++) {
      KeyFrame *pKFi = mit->first;
    ...
    mit++) {
      ...
      if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
        ...
        if (cam0Index != -1 && pKFi->mvuRight[cam0Index] < 0) {
          ...
          nEdges++;
        } else if (cam0Index != -1 && pKFi->mvuRight[cam0Index]
...
const Eigen::Matrix<double, 7, 7>
    matLambda = Eigen::Matrix<double, 7, 7>::Identity();

// Set Loop edges
int count_loop = 0;
for (map<KeyFrame *, set < KeyFrame * > >
::const_iterator
    mit = LoopConnections.begin(), mend = LoopConnections.end();
mit !=
mend;
mit++) {
KeyFrame *pKF = mit->first;
...
count_loop++;
sInsertedEdges.
insert(make_pair(min(nIDi, nIDj), max(nIDi, nIDj))
);
}
}
...
LoopClosing::KeyFrameAndPose::const_iterator
...
const vector<KeyFrame *>
    vpConnectedKFs = pKF->GetCovisiblesByWeight(minFeat);
for (
...
pKFn->mnId<pKF->mnId) {
if (sInsertedEdges.
count(make_pair(min(pKF->mnId, pKFn->mnId),
...
g2o::Sim3 Spw;
LoopClosing::KeyFrameAndPose::const_iterator
    itp = NonCorrectedSim3.find(pKF->mPrevKF);
if (itp != NonCorrectedSim3.
end()
)
Spw = itp->second;
else
...
g2o::VertexSim3Expmap
    *VSim3 = static_cast<g2o::VertexSim3Expmap *>(optimizer.vertex(nIDi));
...
void Optimizer::OptimizeEssentialGraph6DoF(KeyFrame *pCurKF,
  ...
  vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>>
      vScw(nMaxKFid + 1);
  vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>>
      vScw_bef(nMaxKFid + 1);
  vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>>
  ...
  for (KeyFrame *pKFi: vpFixedKFs) {
    ...
    Eigen::Matrix<double, 3, 3>
        Rcw = Converter::toMatrix3d(pKFi->GetRotation());
    Eigen::Matrix<double, 3, 1>
        tcw = Converter::toVector3d(pKFi->GetTranslation());
  ...
  for (KeyFrame *pKFi: vpFixedCorrectedKFs) {
    ...
    Eigen::Matrix<double, 3, 3>
        Rcw_bef = Converter::toMatrix3d(Tcw_bef.rowRange(0, 3).colRange(0, 3));
    Eigen::Matrix<double, 3, 1>
        tcw_bef = Converter::toVector3d(Tcw_bef.rowRange(0, 3).col(3)) / scale;
  ...
  for (KeyFrame *pKFi: vpNonFixedKFs) {
    ...
    Eigen::Matrix<double, 3, 3>
        Rcw = Converter::toMatrix3d(pKFi->GetRotation());
    Eigen::Matrix<double, 3, 1>
        tcw = Converter::toVector3d(pKFi->GetTranslation()) / scale;
    g2o::SE3Quat Siw(Rcw, tcw);
    vScw_bef[nIDi] = Siw;
    VSE3->setEstimate(Siw);
  ...
  vpKFs.insert(vpKFs.end(),
               vpFixedCorrectedKFs.begin(),
  ...
  const Eigen::Matrix<double, 6, 6>
      matLambda = Eigen::Matrix<double, 6, 6>::Identity();
  ...
  for (KeyFrame *pKFi: vpKFs) {
    ...
    // Covisibility graph edges
    const vector<KeyFrame *>
        vpConnectedKFs = pKFi->GetCovisiblesByWeight(minFeat);
  ...
  for (KeyFrame *pKFi: vpNonFixedKFs) {
    ...
    g2o::VertexSE3Expmap
        *VSE3 = static_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(nIDi));
  ...
  for (MapPoint *pMPi: vpNonCorrectedMPs) {
    ...
    Srw =
        vScw_bef[pRefKF->mnId]; //g2o::SE3Quat(RNonCorrectedwr,tNonCorrectedwr).inverse();

    cv::Mat Twr = pRefKF->GetPoseInverse();
    Eigen::Matrix<double, 3, 3>
        Rwr = Converter::toMatrix3d(Twr.rowRange(0, 3).colRange(0, 3));
    Eigen::Matrix<double, 3, 1>
        twr = Converter::toVector3d(Twr.rowRange(0, 3).col(3));
...
void Optimizer::OptimizeEssentialGraph(KeyFrame *pCurKF,
  ...
  for (KeyFrame *pKFi: vpFixedCorrectedKFs) {
    ...
    Eigen::Matrix<double, 3, 3>
        Rcw_bef = Converter::toMatrix3d(Tcw_bef.rowRange(0, 3).colRange(0, 3));
    Eigen::Matrix<double, 3, 1>
        tcw_bef = Converter::toVector3d(Tcw_bef.rowRange(0, 3).col(3));
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1,
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (i2 < 0 && !bAllPoints) {
      Verbose::PrintMess(
          "    Remove point -> i2: " + to_string(i2) + "; bAllPoints: "
              + to_string(bAllPoints), Verbose::VERBOSITY_DEBUG);
      continue;
    }
    ...
    ORB_SLAM3::EdgeInverseSim3ProjectXYZ
        *e21 = new ORB_SLAM3::EdgeInverseSim3ProjectXYZ();
...
void Optimizer::LocalInertialBA(KeyFrame *pKF,
  ...
  for (list<KeyFrame *>::iterator it = lpOptVisKFs.begin(),
           itEnd = lpOptVisKFs.end(); it != itEnd; it++) {
    KeyFrame *pKFi = *it;
    VertexPose *VP = new VertexPose(pKFi);
    VP->setId(pKFi->mnId);
    VP->setFixed(false);
  ...
  for (list<KeyFrame *>::iterator lit = lFixedKeyFrames.begin(),
           lend = lFixedKeyFrames.end(); lit != lend; lit++) {
    KeyFrame *pKFi = *lit;
    VertexPose *VP = new VertexPose(pKFi);
    VP->setId(pKFi->mnId);
    VP->setFixed(true);
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
      ...
      cv::Mat cvInfoG = pKFi->mpImuPreintegrated->C.rowRange(9, 12).colRange(9,
                                                                             12).inv(
          cv::DECOMP_SVD);
      Eigen::Matrix3d InfoG;
      ...
      cv::Mat cvInfoA =
          pKFi->mpImuPreintegrated->C.rowRange(12, 15).colRange(12,
                                                                15).inv(cv::DECOMP_SVD);
      Eigen::Matrix3d InfoA;
      for (int r = 0; r < 3; r++)
        for (int c = 0; c < 3; c++)
          InfoA(r, c) = cvInfoA.at<float>(r, c);
  ...
  // Set MapPoint vertices
  const int
      nExpectedSize = (N + lFixedKeyFrames.size()) * lLocalMapPoints.size();
  ...
  for (int i = 0; i < N; i++) {
    ...
    VertexPose *VP = static_cast<VertexPose *>(optimizer.vertex(pKFi->mnId));
    cv::Mat
        Tcw = Converter::toCvSE3(VP->estimate().Rcw[0], VP->estimate().tcw[0]);
    pKFi->SetPose(Tcw);
    ...
    if (pKFi->bImu) {
      VertexVelocity *VV = static_cast<VertexVelocity *>(optimizer.vertex(
          maxKFid + 3 * (pKFi->mnId) + 1));
      pKFi->SetVelocity(Converter::toCvMat(VV->estimate()));
      VertexGyroBias *VG = static_cast<VertexGyroBias *>(optimizer.vertex(
          maxKFid + 3 * (pKFi->mnId) + 2));
      VertexAccBias *VA = static_cast<VertexAccBias *>(optimizer.vertex(
          maxKFid + 3 * (pKFi->mnId) + 3));
      Vector6d b;
...
Eigen::MatrixXd Optimizer::Marginalize(const Eigen::MatrixXd &H,
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd>::SingularValuesType
...
void Optimizer::InertialOptimization(Map *pMap,
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      g2o::HyperGraph::Vertex *VP1 = optimizer.vertex(pKFi->mPrevKF->mnId);
      g2o::HyperGraph::Vertex
          *VV1 = optimizer.vertex(maxKFid + (pKFi->mPrevKF->mnId) + 1);
      g2o::HyperGraph::Vertex *VP2 = optimizer.vertex(pKFi->mnId);
      g2o::HyperGraph::Vertex
          *VV2 = optimizer.vertex(maxKFid + (pKFi->mnId) + 1);
...
void Optimizer::InertialOptimization(Map *pMap,
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      g2o::HyperGraph::Vertex *VP1 = optimizer.vertex(pKFi->mPrevKF->mnId);
      g2o::HyperGraph::Vertex
          *VV1 = optimizer.vertex((maxKFid + 1) + pKFi->mPrevKF->mnId);
      g2o::HyperGraph::Vertex *VP2 = optimizer.vertex(pKFi->mnId);
      g2o::HyperGraph::Vertex
          *VV2 = optimizer.vertex((maxKFid + 1) + pKFi->mnId);
      g2o::HyperGraph::Vertex
          *VG = optimizer.vertex(2 * (maxKFid + 1) + pKFi->mPrevKF->mnId);
      g2o::HyperGraph::Vertex
          *VA = optimizer.vertex(3 * (maxKFid + 1) + pKFi->mPrevKF->mnId);
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame *pCurrentKF,
  ...
  const int
      nExpectedSize = (vpWeldingKFs.size() + vpFixedKFs.size()) * vpMPs.size();
  ...
  for (unsigned int i = 0; i < vpMPs.size(); ++i) {
    ...
    const map<KeyFrame *, tuple < int, int>>
    observations = pMPi->GetObservations();
    int nEdges = 0;
    //SET EDGES
    for (map < KeyFrame * , tuple < int, int >> ::const_iterator
        mit = observations.begin(); mit != observations.end();
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pMainKF,
  ...
  const int
      nExpectedSize = (vpAdjustKF.size() + vpFixedKF.size()) * vpMPs.size();
...
void Optimizer::MergeInertialBA(KeyFrame *pCurrKF,
  ...
  for (vector<KeyFrame *>::iterator lit = vNeighCurr.begin(),
           lend = vNeighCurr.end(); lit != lend; lit++) {
    if (i >= maxCovKF)
      break;
    KeyFrame *pKFi = *lit;

    if (pKFi->mnBALocalForKF != pCurrKF->mnId && pKFi->mnBAFixedForKF
  ...
  for (vector<KeyFrame *>::iterator lit = vNeighCurr.begin(),
           lend = vNeighCurr.end(); lit != lend; lit++, i++) {
    if (i >= maxCovKF)
      break;
    KeyFrame *pKFi = *lit;
  ...
  for (int i = 0; i < N; i++) {
    ...
    g2o::Sim3
        g2oSiw(Converter::toMatrix3d(Riw), Converter::toVector3d(tiw), 1.0);
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame,
  ...
  egr->setVertex(0, VGk);
  egr->setVertex(1, VG);
  cv::Mat cvInfoG = pFrame->mpImuPreintegrated->C.rowRange(9, 12).colRange(9,
                                                                           12).inv(
      cv::DECOMP_SVD);
  Eigen::Matrix3d InfoG;
  for (int r = 0; r < 3; r++)
  ...
  ear->setVertex(0, VAk);
  ear->setVertex(1, VA);
  cv::Mat cvInfoA = pFrame->mpImuPreintegrated->C.rowRange(12, 15).colRange(12,
                                                                            15).inv(
      cv::DECOMP_SVD);
  Eigen::Matrix3d InfoA;
  for (int r = 0; r < 3; r++)
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit) {
  ...
  egr->setVertex(0, VGk);
  egr->setVertex(1, VG);
  cv::Mat cvInfoG = pFrame->mpImuPreintegratedFrame->C.rowRange(9, 12).colRange(
      9,
      12).inv(cv::DECOMP_SVD);
  Eigen::Matrix3d InfoG;
  for (int r = 0; r < 3; r++)
