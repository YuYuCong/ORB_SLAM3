      ...
      if (bDetected) {
        if (mbMergeDetected) {
          if ((mpTracker->mSensor == System::IMU_MONOCULAR
            ...
            if (mpCurrentKF->GetMap()->IsInertial()
              ...
              {
                Eigen::Vector3d
                    phi = LogSO3(mSold_new.rotation().toRotationMatrix());
                phi(0) = 0;
                phi(1) = 0;
                mSold_new =
        ...
        if (mbLoopDetected) {
          ...
          if (mpCurrentKF->GetMap()->IsInertial()) {
            ...
            Eigen::Vector3d
                phi = LogSO3(g2oSww_new.rotation().toRotationMatrix());
...
bool LoopClosing::NewDetectCommonRegions() {
  ...
  if (mnLoopNumCoincidences > 0) {
    ...
    cv::Mat
        mTcl = mpCurrentKF->GetPose() * mpLoopLastCurrentKF->GetPoseInverse();
  ...
  if (mnMergeNumCoincidences > 0) {
    // Find from the last KF candidates
    cv::Mat
        mTcl = mpCurrentKF->GetPose() * mpMergeLastCurrentKF->GetPoseInverse();
    g2o::Sim3 gScl(Converter::toMatrix3d(mTcl.rowRange(0, 3).colRange(0, 3)),
                   Converter::toVector3d(mTcl.rowRange(0, 3).col(3)),
                   1.0);
...
bool LoopClosing::DetectCommonRegionsFromBoW(std::vector<KeyFrame *> &vpBowCand,
  ...
  for (KeyFrame *pKFi: vpBowCand) {
    ...
    std::vector<KeyFrame *>
        vpCovKFi = pKFi->GetBestCovisibilityKeyFrames(nNumCovisibles);
      ...
      if (bConverge) {
        ...
        if (numProjMatches >= nProjMatches) {
          ...
          if (numOptMatches >= nSim3Inliers) {
            ...
            vpMatchedMP.resize(mpCurrentKF->GetMapPointMatches().size(),
                               static_cast<MapPoint *>(NULL));
            int numProjOptMatches = matcher.SearchByProjection(mpCurrentKF,
                                                               mScw,
                                                               vpMapPoints,
                                                               vpMatchedMP,
                                                               5,
...
int LoopClosing::FindMatchesByProjection(KeyFrame *pCurrentKF,
  ...
  int nNumCovisibles = 5;
  vector<KeyFrame *>
      vpCovKFm = pMatchedKFw->GetBestCovisibilityKeyFrames(nNumCovisibles);
  ...
  for (int i = 0; i < nInitialCov; ++i) {
    vector<KeyFrame *>
        vpKFs = vpCovKFm[i]->GetBestCovisibilityKeyFrames(nNumCovisibles);
    int nInserted = 0;
    int j = 0;
...
void LoopClosing::CorrectLoop() {
...

  {
    ...
    for (vector<KeyFrame *>::iterator vit = mvpCurrentConnectedKFs.begin(),
      ...
      if (pKFi != mpCurrentKF) {
        ...
        g2o::Sim3
            g2oSic(Converter::toMatrix3d(Ric), Converter::toVector3d(tic), 1.0);
    ...
    for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
      ...
      if (bImuInit) {
        Eigen::Matrix3d
            Rcor = eigR.transpose() * g2oSiw.rotation().toRotationMatrix();
  ...
  for (vector<KeyFrame *>::iterator vit = mvpCurrentConnectedKFs.begin(),
    ...
    for (vector<KeyFrame *>::iterator vit_prev = vpPreviousNeighbors.begin(),
             vend_prev = vpPreviousNeighbors.end(); vit_prev != vend_prev;
         vit_prev++) {
...
void LoopClosing::MergeLocal() {
  ...
  while (spLocalWindowKFs.size() < numTemporalKFs && nNumTries < nMaxTries) {
    ...
    for (KeyFrame *pKFi: spLocalWindowKFs) {
      vector<KeyFrame *>
          vpKFiCov = pKFi->GetBestCovisibilityKeyFrames(numTemporalKFs / 2);
  ...
  for (MapPoint *pMPi: spLocalWindowMPs) {
    ...
    Eigen::Matrix<double, 3, 1> eigP3Dw = Converter::toVector3d(P3Dw);
    Eigen::Matrix<double, 3, 1>
        eigCorrectedP3Dw = g2oCorrectedSwi.map(g2oNonCorrectedSiw.map(eigP3Dw));
    Eigen::Matrix3d eigR = g2oCorrectedSwi.rotation().toRotationMatrix();
    Eigen::Matrix3d
        Rcor = eigR * g2oNonCorrectedSiw.rotation().toRotationMatrix();
  ...
  if (vpCurrentMapKFs.size() == 0) {
    Verbose::PrintMess("MERGE: There are not KFs outside of the welding area",
                       Verbose::VERBOSITY_DEBUG);
  } else {
    Verbose::PrintMess(
        "MERGE: Calculate the new position of the elements outside of the window",
        Verbose::VERBOSITY_DEBUG);
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap,
  ...
  for (KeyFrameAndPose::const_iterator mit = CorrectedPosesMap.begin(),
    ...
    vector<MapPoint *>
        vpReplacePoints(vpMapPoints.size(), static_cast<MapPoint *>(NULL));
