...
MapPoint::MapPoint(const cv::Mat &Pos,
  ...
  mNormalVectorx = cv::Matx31f(mNormalVector.at<float>(0),
                               mNormalVector.at<float>(1),
...
void MapPoint::ComputeDistinctiveDescriptors() {
  ...
  std::vector<std::vector<float> > Distances(N, std::vector<float>(N)); // NxN
  ...
  int BestIdx = 0;
  for (size_t i = 0; i < N; i++) {
    std::vector<float> vDists = Distances[i];
