...
Sim3Solver::Impl::Impl(KeyFrame *pKF1,
                       KeyFrame *pKF2,
...
void Sim3Solver::Impl::SetRansacParameters(double probability,
                                           int minInliers,
                                           int maxIterations) {
...
cv::Mat Sim3Solver::Impl::iterate(int nIterations,
                                  bool &bNoMore,
                                  vector<bool> &vbInliers,
                                  int &nInliers) {
  bNoMore = false;
...
cv::Mat Sim3Solver::Impl::find(vector<bool> &vbInliers12, int &nInliers) {
  bool bFlag;
  return iterate(mRansacMaxIts, bFlag, vbInliers12, nInliers);
}

void Sim3Solver::Impl::ComputeCentroid(cv::Mat &P, cv::Mat &Pr, cv::Mat &C) {
...
void Sim3Solver::Impl::ComputeSim3(cv::Mat &P1, cv::Mat &P2) {
  ...
  (evec.row(0).colRange(1,
                        4)).copyTo(vec); //extract imaginary part of the quaternion (sin*axis)
...
void Sim3Solver::Impl::CheckInliers() {
...
cv::Mat Sim3Solver::Impl::GetEstimatedRotation() {
  return mBestRotation.clone();
}

cv::Mat Sim3Solver::Impl::GetEstimatedTranslation() {
  return mBestTranslation.clone();
}

float Sim3Solver::Impl::GetEstimatedScale() {
  return mBestScale;
}

void Sim3Solver::Impl::Project(const vector<cv::Mat> &vP3Dw,
                               vector<cv::Mat> &vP2D,
                               cv::Mat Tcw,
                               GeometricCamera *pCamera) {
...
void Sim3Solver::Impl::FromCameraToImage(const vector<cv::Mat> &vP3Dc,
                                         vector<cv::Mat> &vP2D,
                                         GeometricCamera *pCamera) {
  vP2D.clear();
...
/*** Implementation of Sim3Solver ***/
Sim3Solver::Sim3Solver(KeyFrame *pKF1,
                       KeyFrame *pKF2,
                       const std::vector<MapPoint *> &vpMatched12,
                       const bool bFixScale,
                       const vector<KeyFrame *> vpKeyFrameMatchedMP)
    : impl_(std::make_shared<Impl>(pKF1,
                                   pKF2,
                                   vpMatched12,
                                   bFixScale,
                                   vpKeyFrameMatchedMP)) {
...
void Sim3Solver::SetRansacParameters(double probability,
                                     int minInliers,
                                     int maxIterations) {
  return impl_->SetRansacParameters(probability, minInliers, maxIterations);
}

cv::Mat Sim3Solver::find(std::vector<bool> &vbInliers12, int &nInliers) {
  return impl_->find(vbInliers12, nInliers);
}
...
cv::Mat Sim3Solver::iterate(int nIterations,
                            ...
                            int &nInliers,
                            bool &bConverge) {
  return impl_->iterate(nIterations, bNoMore, vbInliers, nInliers, bConverge);
}

cv::Mat Sim3Solver::GetEstimatedRotation() {
  return impl_->GetEstimatedRotation();
}

cv::Mat Sim3Solver::GetEstimatedTranslation() {
  return impl_->GetEstimatedTranslation();
}

float Sim3Solver::GetEstimatedScale() { return impl_->GetEstimatedScale(); }
