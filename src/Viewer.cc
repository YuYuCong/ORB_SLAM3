...
Viewer::Impl::Impl(System *pSystem,
                   FrameDrawer *pFrameDrawer,
                   MapDrawer *pMapDrawer,
                   Tracking *pTracking,
                   const string &strSettingPath) :
...
bool Viewer::Impl::ParseViewerParamFile(cv::FileStorage &fSettings) {
...
void Viewer::Impl::Run() {
  ...
  pangolin::Var<bool>
      menuShowInertialGraph("menu.Show Inertial Graph", true, true);
  pangolin::Var<bool>
...
void Viewer::Impl::RequestFinish() {
  unique_lock<mutex> lock(mMutexFinish);
  mbFinishRequested = true;
}

bool Viewer::Impl::CheckFinish() {
  unique_lock<mutex> lock(mMutexFinish);
  return mbFinishRequested;
}

void Viewer::Impl::SetFinish() {
  unique_lock<mutex> lock(mMutexFinish);
  mbFinished = true;
}
...
void Viewer::Impl::RequestStop() {
  unique_lock<mutex> lock(mMutexStop);
  if (!mbStopped)
    mbStopRequested = true;
}

bool Viewer::Impl::isStopped() {
  unique_lock<mutex> lock(mMutexStop);
  return mbStopped;
}

bool Viewer::Impl::Stop() {
  unique_lock<mutex> lock(mMutexStop);
...
bool Viewer::Impl::isStepByStep() {
  return true;
}

void Viewer::Impl::Release() {
  unique_lock<mutex> lock(mMutexStop);
  mbStopped = false;
}

void Viewer::Impl::SetTrackingPause() {
  mbStopTrack = true;
}
...
Viewer::Viewer(System *pSystem,
               ...
               Tracking *pTracking,
               const string &strSettingPath) :
    impl_(std::make_shared<Impl>(pSystem,
                                 pFrameDrawer,
                                 pMapDrawer,
                                 pTracking,
                                 strSettingPath)) {
