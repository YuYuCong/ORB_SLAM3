...
class ORBmatcher::Impl {
 public:
  Impl(float nnratio = 0.6, bool checkOri = true);
  ...
  int SearchForTriangulation_(KeyFrame *pKF1,
                              KeyFrame *pKF2,
                              cv::Matx33f F12,
                              std::vector<pair<size_t, size_t> > &vMatchedPairs,
                              const bool bOnlyStereo,
...
ORBmatcher::Impl::Impl(float nnratio, bool checkOri)
    : mfNNratio(nnratio), mbCheckOrientation(checkOri) {
}

int ORBmatcher::Impl::SearchByProjection(Frame &F,
                                         const vector<MapPoint *> &vpMapPoints,
                                         const float th,
...
float ORBmatcher::Impl::RadiusByViewingCos(const float &viewCos) {
  if (viewCos > 0.998)
    return 2.5;
  else
    return 4.0;
...
bool ORBmatcher::Impl::CheckDistEpipolarLine(const cv::KeyPoint &kp1,
                                             const cv::KeyPoint &kp2,
                                             const cv::Mat &F12,
                                             const KeyFrame *pKF2,
                                             const bool b1) {
  ...
  const float a =
      kp1.pt.x * F12.at<float>(0, 0) + kp1.pt.y * F12.at<float>(1, 0)
          + F12.at<float>(2, 0);
  const float b =
      kp1.pt.x * F12.at<float>(0, 1) + kp1.pt.y * F12.at<float>(1, 1)
          + F12.at<float>(2, 1);
  const float c =
      kp1.pt.x * F12.at<float>(0, 2) + kp1.pt.y * F12.at<float>(1, 2)
          + F12.at<float>(2, 2);
...
bool ORBmatcher::Impl::CheckDistEpipolarLine2(const cv::KeyPoint &kp1,
                                              const cv::KeyPoint &kp2,
                                              const cv::Mat &F12,
                                              const KeyFrame *pKF2,
                                              const float unc) {
...
int ORBmatcher::Impl::SearchByBoW(KeyFrame *pKF,
                                  Frame &F,
        ...
        if (bestDist1 <= TH_LOW) {
          if (static_cast<float>(bestDist1)
            ...
            const cv::KeyPoint &kp =
                ...
                (realIdxKF >= pKF->NLeft) ? pKF->mvKeysRight[realIdxKF
                    - pKF->NLeft]
          ...
          if (bestDist1R <= TH_LOW) {
            if (static_cast<float>(bestDist1R)
                < mfNNratio * static_cast<float>(bestDist2R) || true) {
...
int ORBmatcher::Impl::SearchByProjection(KeyFrame *pKF,
                                         cv::Mat Scw,
...
int ORBmatcher::Impl::SearchForInitialization(Frame &F1,
                                              Frame &F2,
...
int ORBmatcher::Impl::SearchByBoW(KeyFrame *pKF1,
                                  KeyFrame *pKF2,
...
int ORBmatcher::Impl::SearchForTriangulation(KeyFrame *pKF1,
                                             KeyFrame *pKF2,
                                             cv::Mat F12,
                                             vector<pair<size_t,
  ...
  while (f1it != f1end && f2it != f2end) {
    if (f1it->first == f2it->first) {
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        if (bestIdx2 >= 0) {
          const cv::KeyPoint
              &kp2 = (pKF2->NLeft == -1) ? pKF2->mvKeysUn[bestIdx2]
                                         : (bestIdx2 < pKF2->NLeft)
                                           ? pKF2->mvKeys[bestIdx2]
                                           : pKF2->mvKeysRight[bestIdx2
                      - pKF2->NLeft];
...
int ORBmatcher::Impl::SearchForTriangulation_(KeyFrame *pKF1,
                                              KeyFrame *pKF2,
                                              cv::Matx33f F12,
                                              vector<pair<size_t,
                                                          size_t> > &vMatchedPairs,
  ...
  if (!pKF1->mpCamera2 && !pKF2->mpCamera2) {
    ...
    tlr = pKF1->GetRotation_()
        * (-pKF2->GetRightRotation_().t() * pKF2->GetRightTranslation_())
        + pKF1->GetTranslation_();
    trl = pKF1->GetRightRotation_()
        * (-pKF2->GetRotation_().t() * pKF2->GetTranslation_())
        + pKF1->GetRightTranslation_();
    trr = pKF1->GetRightRotation_()
        * (-pKF2->GetRightRotation_().t() * pKF2->GetRightTranslation_())
        + pKF1->GetRightTranslation_();
  }
    ...
    if (f1it->first == f2it->first) {
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          if (pCamera1->epipolarConstrain_(pCamera2,
                                           kp1,
                                           kp2,
                                           R12,
                                           t12,
...
int ORBmatcher::Impl::Fuse(KeyFrame *pKF,
                           const vector<MapPoint *> &vpMapPoints,
                           const float th,
                           const bool bRight) {
  cv::Mat Rcw, tcw, Ow;
  ...
  for (int i = 0; i < nMPs; i++) {
    ...
    const vector<size_t>
        vIndices = pKF->GetFeaturesInArea(uv.x, uv.y, radius, bRight);
...
int ORBmatcher::Impl::SearchBySim3(KeyFrame *pKF1,
                                   KeyFrame *pKF2,
                                   vector<MapPoint *> &vpMatches12,
                                   const float &s12,
                                   const cv::Mat &R12,
...
int ORBmatcher::Impl::SearchByProjection(Frame &CurrentFrame,
                                         const Frame &LastFrame,
                                         const float th,
                                         const bool bMono) {
  int nmatches = 0;
    ...
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) {
        ...
        if (bForward)
          ...
          vIndices2 = CurrentFrame.GetFeaturesInArea(uv.x,
                                                     uv.y,
                                                     radius,
                                                     0,
                                                     nLastOctave);
        ...
        if (CurrentFrame.Nleft != -1) {
          ...
          for (vector<size_t>::const_iterator vit = vIndices2.begin(),
            ...
            if (CurrentFrame.mvpMapPoints[i2 + CurrentFrame.Nleft])
              if (CurrentFrame.mvpMapPoints[i2
                  + CurrentFrame.Nleft]->Observations() > 0)
                continue;

            const cv::Mat
                &d = CurrentFrame.mDescriptors.row(i2 + CurrentFrame.Nleft);
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
    ...
    if (pMP) {
      if (!pMP->isBad() && !sAlreadyFound.count(pMP)) {
        ...
        const vector<size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(uv.x,
                                                                        uv.y,
                                                                        radius,
                                                                        nPredictedLevel
                                                                            - 1,
...
void ORBmatcher::Impl::ComputeThreeMaxima(vector<int> *histo,
                                          const int L,
                                          int &ind1,
                                          int &ind2,
                                          int &ind3) {
...
ORBmatcher::ORBmatcher(float nnratio, bool checkOri)
    : impl_(std::make_shared<Impl>(nnratio, checkOri)) {}
...
int ORBmatcher::SearchByProjection(Frame &F,
                                   ...
                                   const bool bFarPoints,
                                   const float thFarPoints) {
  return impl_->SearchByProjection(F, vpMapPoints, th, bFarPoints, thFarPoints);
}
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame,
                                   ...
                                   const float th,
                                   const bool bMono) {
  return impl_->SearchByProjection(CurrentFrame, LastFrame, th, bMono);
}
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame,
                                   ...
                                   const float th,
                                   const int ORBdist) {
  return impl_->SearchByProjection(CurrentFrame,
                                   pKF,
                                   sAlreadyFound,
                                   th,
                                   ORBdist);
...
int ORBmatcher::SearchByProjection(KeyFrame *pKF,
                                   ...
                                   int th,
                                   float ratioHamming) {
  return impl_->SearchByProjection(pKF,
                                   Scw,
                                   vpPoints,
                                   vpMatched,
                                   th,
...
int ORBmatcher::SearchByBoW(KeyFrame *pKF,
                            Frame &F,
                            std::vector<MapPoint *> &vpMapPointMatches) {
  return impl_->SearchByBoW(pKF, F, vpMapPointMatches);
}

int ORBmatcher::SearchByBoW(KeyFrame *pKF1,
                            KeyFrame *pKF2,
                            std::vector<MapPoint *> &vpMatches12) {
  return impl_->SearchByBoW(pKF1, pKF2, vpMatches12);
}
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2,
                                        ...
                                        std::vector<int> &vnMatches12,
                                        int windowSize) {
  return impl_->SearchForInitialization(F1, F2, vbPrevMatched, vnMatches12,
                                        windowSize);
}
...
int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1,
                                       KeyFrame *pKF2,
                                       cv::Mat F12,
                                       std::vector<pair<size_t,
                                                        size_t> > &vMatchedPairs,
                                       const bool bOnlyStereo,
                                       const bool bCoarse) {
  return impl_->SearchForTriangulation(pKF1, pKF2, F12, vMatchedPairs,
                                       bOnlyStereo, bCoarse);
}

int ORBmatcher::SearchForTriangulation_(KeyFrame *pKF1,
                                        KeyFrame *pKF2,
                                        cv::Matx33f F12,
                                        std::vector<pair<size_t,
                                                         size_t> > &vMatchedPairs,
                                        const bool bOnlyStereo,
                                        const bool bCoarse) {
  return impl_->SearchForTriangulation_(pKF1, pKF2, F12, vMatchedPairs,
                                        bOnlyStereo, bCoarse);
}
