...
cv::Mat LogSO3(const cv::Mat &R) {
  ...
  cv::Mat
      w = (cv::Mat_<float>(3, 1) << (R.at<float>(2, 1) - R.at<float>(1, 2)) / 2,
...
cv::Mat InverseRightJacobianSO3(const cv::Mat &v) {
  return InverseRightJacobianSO3(v.at<float>(0),
                                 v.at<float>(1),
                                 v.at<float>(2));
}
...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration,
  ...
  cv::Mat acc =
      (cv::Mat_<float>(3, 1) << acceleration.x - b.bax, acceleration.y - b.bay,
          acceleration.z - b.baz);
  cv::Mat accW =
      (cv::Mat_<float>(3, 1) << angVel.x - b.bwx, angVel.y - b.bwy, angVel.z
          - b.bwz);
  ...
  cv::Mat
      Wacc = (cv::Mat_<float>(3, 3) << 0, -acc.at<float>(2), acc.at<float>(1),
