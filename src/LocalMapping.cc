...
void LocalMapping::CreateNewMapPoints() {
  ...
  if (mbInertial) {
    ...
    while ((vpNeighKFs.size() <= nn) && (pKF->mPrevKF) && (count++ < nn)) {
      vector<KeyFrame *>::iterator
          it = std::find(vpNeighKFs.begin(), vpNeighKFs.end(), pKF->mPrevKF);
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    matcher.SearchForTriangulation_(mpCurrentKeyFrame,
                                    pKF2,
                                    F12,
                                    vMatchedIndices,
                                    false,
...
void LocalMapping::SearchInNeighbors() {
  ...
  for (int i = 0, imax = vpTargetKFs.size(); i < imax; i++) {
    const vector<KeyFrame *>
        vpSecondNeighKFs = vpTargetKFs[i]->GetBestCovisibilityKeyFrames(20);
        ...
        if (!pMP->isBad()) {
          ...
          if (pMP->Observations() > thObs) {
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator
                     mit = observations.begin(), mend = observations.end();
                 mit != mend; mit++) {
